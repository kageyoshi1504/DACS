<div style="padding-left: 15px; padding-top: 10px; padding-right: 15px">
        ...
        foreach($dataPurchase as $line){
            ...
            <tr style="background:<?php if($dem%2 == 1) echo '#efefef'; else echo 'fcfcfc';?>">
                ...
                <td style="text-align: center">
                    <?php
                    if($line['tinhtrangthanhtoan']==0) echo "<span style='; color: #c82333; padding: 0px 5px'>NO</span>";
                    else if($line['tinhtrangthanhtoan']==1) echo "<span style='color: #28a745; padding: 0px 5px'>YES</span>";
                    ?>
                </td>
                <td>
                    <?php
    ...
    <dialog id="duyet" style="padding: 10px 15px;border: none; width: 400px;  box-shadow: 0px 0px 15px 0px #aaa; border-radius: 5px; display: none; position: fixed; top:50%;left: 50%;  transform: translate(-50%,-50%);">
        <p style="font-size: 16px;">Nếu bạn đã chắc chắn rằng đơn hàng này đã được gửi đi, hãy bấm xác nhận.</p>
        <b>Xác nhận đã gửi đơn hàng</b>
        ...
        <form style="float: right" method="post" action="#">
            ...
            <button style="padding: 2px; font-size: 14px" class="btn btn-info" id="xacnhandelete">Xác nhận đã gửi hàng</button>
        </form>
    </dialog>
